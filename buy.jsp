...
rst=stmt.executeQuery("select productname,productprice,quantity,discount,productid from product where productname='"+pname+"' ");
...
float res=ch/100;
 ...
 // out.print("product  amount"+productAmt);
  ...
  <table width="568" border="1" align="center">
    ...
    <tr>
      ...
      <td><p align="right">card holder Email
          <input type="text" name="chname" value="<%= session.getAttribute("customername") %>" readonly="true"/>
      </p>
  ...
  <%  
	...
	int x=stmt.executeUpdate("insert into orderlist (productname,productCast,quantity,totalAmount,address,cardHolderName,cardtype,cardNum,cvv,charity)values('"+t1+"','"+t2+"','"+t3 +"','"+t4+"','"+t5+"','"+t6+"','"+t7+"','"+t8+"','"+t9+"',"+t10+")");
	if(x!=0){
